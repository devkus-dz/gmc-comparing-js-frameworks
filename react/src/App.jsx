...
function App() {
  ...
  /**
   * Wraps state updates to measure the exact time taken for React to commit 
   * and the browser to paint the resulting DOM updates.
   * @param {Function} updateFn - The function containing the React state mutation.
   */
  const measureDOMUpdate = (updateFn) => {
    ...
    requestAnimationFrame(() => {
      setTimeout(() => {
        ...
        if (timeDisplayRef.current) {
          timeDisplayRef.current.textContent = (end - start).toFixed(2);
        }
      }, 0);
    });
  };
    ...
    <div className="container mx-auto p-6 max-w-6xl space-y-8">
      ...
      <div className="p-4 bg-slate-100 rounded-lg border border-slate-200 space-y-4">
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <Button variant="ghost" onClick={() => measureDOMUpdate(() => setTasks([]))}>Clear</Button>
        </div>
